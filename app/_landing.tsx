...
import { htmlLang } from "@/dictionaries";
import { cjkFontClass } from "./_fonts";
...
const PLUGIN_REPO_URL = "https://github.com/je0ng3/vibi-adobe-plugin";
...
export function LandingPage({ dict, lang }: { dict: Dict; lang: Locale }) {
  // For CJK locales, insert the language's Noto fonts into the type stacks as a
  // fallback after the latin fonts — latin brand words stay in Inter/Fraunces,
  // CJK glyphs render in Noto Sans/Serif. Latin locales are left untouched.
  const cjkClass = cjkFontClass[lang];
  const cjkStyle: CSSProperties | undefined = cjkClass
    ? ({
        "--font-display": 'var(--font-display-latin), var(--font-display-cjk), "Times New Roman", serif',
        "--font-body":
          "var(--font-body-latin), var(--font-body-cjk), -apple-system, BlinkMacSystemFont, \"Segoe UI\", sans-serif",
      } as CSSProperties)
    : undefined;

  return (
    <main
      className={`relative${cjkClass ? ` ${cjkClass}` : ""}`}
      lang={htmlLang[lang]}
      style={cjkStyle}
    >
      <JsonLd dict={dict} />
...
function JsonLd({ dict }: { dict: Dict }) {
  ...
  const graph = [
    ...
    },
    {
      ...
      "@id": `${site}/#app-ios`,
      name: "VIBI: AI Sound Eraser",
      alternateName: "VIBI",
      description: dict.meta.description,
      applicationCategory: "MultimediaApplication",
      applicationSubCategory: "Photo & Video",
      operatingSystem: "iOS 17.2, Android",
      ...
      featureList: dict.jsonLd.featureList,
